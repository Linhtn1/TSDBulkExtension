...
/// <summary>Staged operations: temp table plus set statements, generated values mapped back by row index.</summary>
public abstract class MergeTestsBase : ProviderTestBase
{
    // ---- Insert with output ---------------------------------------------------------------------------------------

    [Fact]
    public async Task Insert_with_SetOutputIdentity_writes_ids_back_in_list_order()
    {
        var tag = Tag("mrg");
    ...
    [Fact]
    public async Task Update_with_identical_values_does_not_bump_the_rowversion()
    {
        var tag = Tag("mrg");
        ...
        var rows = await LoadAsync(tag);
        Assert.Equal(new[] { 1, 2 }, rows.Select(r => r.Quantity));
        if (SupportsRowVersion)
        {
    ...
    [Fact]
    public async Task Update_with_identical_values_reports_no_updates()
    {
        var tag = Tag("mrg");
        var items = await SeedAsync(tag, 2);
        ...
        await using (var context = CreateContext())
        {
            await context.BulkUpdateAsync(items, config);
        }
    ...
    [Fact]
    public async Task Upsert_with_a_nullable_match_key_matches_null_to_null()
    {
        List<SyncRow> existing;
        await using (var context = CreateContext())
        {
            await context.TruncateAsync<SyncRow>();
            existing = new List<SyncRow> { new() { Key = "a", Category = null, Value = 1 }, new() { Key = "b", Category = "x", Value = 2 } };
            context.SyncRows.AddRange(existing);
            await context.SaveChangesAsync();
        }

        var incoming = new List<SyncRow>
        {
            new() { Key = "a2", Category = null, Value = 10 },
            new() { Key = "b2", Category = "x", Value = 20 },
            new() { Key = "c", Category = "y", Value = 30 },
        };
        var config = new BulkConfig
        {
            UpdateByProperties = new List<string> { nameof(SyncRow.Category) },
            PropertiesToIncludeOnUpdate = new List<string> { nameof(SyncRow.Value) },
        ...
        await using (var context = CreateContext())
        {
            await context.BulkInsertOrUpdateAsync(incoming, config);
        }

        Assert.Equal(existing[0].Id, incoming[0].Id);
        Assert.Equal(existing[1].Id, incoming[1].Id);
        Assert.True(incoming[2].Id > existing[1].Id);
        ...
        await using (var context = CreateContext())
        {
            ...
            Assert.Equal(new[] { ("a", 10), ("b", 20), ("c", 30) }, rows.Select(r => (r.Key, r.Value)));
        }
    }
    ...
    public async Task Upsert_with_nothing_to_update_still_returns_ids_of_matched_rows()
    {
        ...
        await using (var context = CreateContext())
        {
            // The update list resolves to the key only, so the UPDATE branch has no columns; matched rows must still get their ids.
            await context.BulkInsertOrUpdateAsync(incoming, opt =>
            {
                opt.UpdateByProperties = new List<string> { nameof(Item.Name) };
    ...
    [Fact]
    public async Task Upsert_with_KeepIdentity_inserts_explicit_ids_and_keeps_the_identity_ahead_of_them()
    {
        var tag = Tag("mrg");
        long baseId;
        await using (var context = CreateContext())
        {
            baseId = (await context.Items.MaxAsync(i => (long?)i.Id) ?? 0) + 1;
        }
        ...
        // The identity/sequence must have moved past the explicit values, or the next ordinary insert collides.
        var next = await SeedAsync(tag, 1);
        Assert.True(next[0].Id > baseId + 1, $"next id {next[0].Id} should be above {baseId + 1}");
    }

    [Fact]
    public async Task Sync_on_the_identity_key_keeps_the_rows_it_inserts()
    {
        List<SyncRow> existing;
        await using (var context = CreateContext())
        {
            await context.TruncateAsync<SyncRow>();
            existing = new List<SyncRow> { new() { Key = "a", Value = 1 }, new() { Key = "b", Value = 2 } };
            context.SyncRows.AddRange(existing);
            await context.SaveChangesAsync();
        }
        ...
        await using (var context = CreateContext())
        {
            // Default match on the identity key, no SetOutputIdentity: the new row gets its key from the server during the insert.
            await context.BulkInsertOrUpdateOrDeleteAsync(new List<SyncRow>
            {
                new() { Id = existing[0].Id, Key = "a", Value = 10 },
                new() { Key = "d", Value = 4 },
            }, config);
        }
        ...
        Assert.Equal(1, config.StatsInfo!.StatsNumberInserted);
        Assert.Equal(1, config.StatsInfo.StatsNumberUpdated);
        Assert.Equal(1, config.StatsInfo.StatsNumberDeleted);
    }
    ...
    public async Task Ambiguous_match_rolls_back_the_merge_when_no_transaction_was_active()
    {
        ...
        await using (var context = CreateContext())
        {
            ...
            var second = await context.Items.SingleAsync(i => i.Id == seeded[1].Id);
            second.Quantity = 1;
            await context.SaveChangesAsync();
        }
        ...
        // The statement itself updated both rows, but the executor's own transaction rolled it back.
