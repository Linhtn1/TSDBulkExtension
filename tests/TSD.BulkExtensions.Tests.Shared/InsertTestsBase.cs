...
/// <summary>Fast-path BulkInsert (no SetOutputIdentity): the provider's streaming API straight into the table.</summary>
public abstract class InsertTestsBase : ProviderTestBase
{
    [Fact]
    ...
    [Fact]
    public async Task KeepIdentity_preserves_explicit_ids_and_keeps_the_identity_ahead_of_them()
    {
        var tag = Tag("ins");
        long baseId;
        await using (var context = CreateContext())
        ...
        Assert.True(next[0].Id > baseId + 2, $"next id {next[0].Id} should be above {baseId + 2}");
    }

    [Fact]
    public async Task Single_batch_reports_progress_once()
    {
        var tag = Tag("ins");
        var reported = new List<decimal>();
        ...
        await using (var context = CreateContext())
        {
            await context.BulkInsertAsync(Items(tag, 50), opt => opt.BatchSize = 0, progress: reported.Add);
        }

        Assert.Equal(new[] { 1m }, reported);
        Assert.Equal(50, (await LoadAsync(tag)).Count);
    }
