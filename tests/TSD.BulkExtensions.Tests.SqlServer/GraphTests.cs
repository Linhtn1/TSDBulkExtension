...
public sealed class GraphTests : SqlServerTestBase
{
    public GraphTests(SqlServerFixture fixture) : base(fixture)
    {
    }
    ...
    public async Task Failure_in_a_pass_rolls_back_the_whole_graph()
    {
        ...
        await using (var context = CreateContext())
        {
            await Assert.ThrowsAsync<SqlException>(() => context.BulkInsertAsync(orders, opt => opt.IncludeGraph = true));
        }
